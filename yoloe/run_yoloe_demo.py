...
from yoloe.yoloe_model import YOLOEDemoRunner
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("yoloe/yoloe_config.yaml"),
    ...
    runner = YOLOEDemoRunner(weights=config.get("weights", "models/yoloe-11s-seg.pt"))
    outputs = runner.predict(
        ...
        output_dir=resolve_path(config.get("output_dir", "result/yoloe_demo")),
    )
